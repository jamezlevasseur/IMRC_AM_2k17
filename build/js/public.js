...
(function ($) {
	...
	$(function () {
		...
		var newDataToRefLeft = function newDataToRefLeft(data, empty) {
		...
		var init_res_delete = function init_res_delete() {
		...
		var reload_existing_res = function reload_existing_res() {
		...
		var initCrumbListener = function initCrumbListener() {
			$('.iam-crumb').click(function (event) {
				var crumbIndex = breadcrumbTrail.indexOf($(this).text());
				if (crumbIndex == -1 || crumbIndex == breadcrumbTrail.length - 1) return;
		...
		var initCrumbButtonListener = function initCrumbButtonListener() {
		...
		var updatePageForCrumb = function updatePageForCrumb() {
			if (breadcrumbTrail.length < 1) return;
			$('.iam-ref-left').empty();
			$.ajax({
				url: ajaxurl,
		...
		var popToCrumbRoot = function popToCrumbRoot() {
			//clear crumbs
			breadcrumbTrail = [];
			$('#iam-ref-crumb').empty();
			//clear page
		...
		var initRootCrumbListener = function initRootCrumbListener() {
		...
		var preventPastReservation = function preventPastReservation(e, businessHoursConverted) {
		...
		var warnIfOutOfBounds = function warnIfOutOfBounds(e, businessHoursConverted) {
		...
		var emptyBlockBuffer = function emptyBlockBuffer() {
    ...
    */
		...
		if ($('#iam-ref').length > 0) {
			...
			$.ajax({
				...
				success: function success(data) {
					res_form = (0, _serverresponse.handleServerResponse)(data);
				},
				error: function error(data) {
					(0, _serverresponse.handleServerError)(data, new Error());
				}
			...
			$('#register-submit').click(function (event) {
				...
				var registerArgs = { action: 'iam_register_user', 'email': $('#email').val(), 'first-name': $('#first-name').val(), 'last-name': $('#last-name').val(), 'account_type': $('#account_type').val(), 'student-id': $('#student-id').val(), phonenum: (0, _utils.getPhoneNumberFromPage)(), 'password': $('#password').val(), 'key': $('#reg-key').val(), captcha: grecaptcha.getResponse() };
...
function registerDebug() {
  if ((0, _jquery2.default)('#signupform').length > 0) {
    (0, _jquery2.default)('body').dblclick(function (event) {
      ...
      (0, _jquery2.default)('#student-id').val((0, _utils.rStr)(16));
