...
function EventManager() { // assumed to be a calendar
	...
	function getBusinessHoursEvents(wholeDay) {
		...
		if ($.isArray(optionVal)) {
			var allBusinessHourEvents = [];
			for (var o in optionVal) {
				allBusinessHourEvents = allBusinessHourEvents.concat(makeBusinessHoursEvents(wholeDay, optionVal[o]));
			}
			return allBusinessHourEvents;
		}
		else {
			return makeBusinessHoursEvents(wholeDay, optionVal);
		}

		return [];
	}

	function makeBusinessHoursEvents (wholeDay, optionVal) {
		var defaultVal = {
	...
	function constraintToEvents(constraintInput) {
		...
		if ($.isArray(constraintInput)) {
			var allConstraints = [];
			for (var c in constraintInput) {
				allConstraints = allConstraints.concat(expandEvent(buildEventFromInput(constraintInput[c])));
			}
